...
class FuelProviderOrderRepositoryImpl implements IFuelProviderOrderRepository {
  ...
  Future<Either<Failure, FuelOrderEntity>> startOrder(int id) =>
      _call(() async => _mapOrder((await _remote.startOrder(id)).data));
  ...
  Future<Either<Failure, FuelOrderEntity>> cancelOrder(int id, String reason) =>
      _call(
        () async => _mapOrder((await _remote.cancelOrder(id, reason)).data),
      );

  @override
  Future<Either<Failure, List<FuelOrderEntity>>> getMyOrders() => _call(
    () async => (await _remote.getMyOrders()).data.map(_mapOrder).toList(),
  );
  @override
  Future<Either<Failure, List<FuelOrderEntity>>> getavailableOrders() => _call(
    () async =>
        (await _remote.getavailableOrders()).data.map(_mapOrder).toList(),
  );
  @override
